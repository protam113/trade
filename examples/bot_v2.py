...
# Strategy Parameters
TIMEFRAME = mt5.TIMEFRAME_M1  # Changed to M1 (1-minute candles)
MACD_FAST = 12
MACD_SLOW = 26
MACD_SIGNAL = 9
TP_PIPS = 30
...
def place_order(symbol, order_type, lot_size, tp_pips, sl_pips):
    ...
    request = {
        ...
        "deviation": 20,
        "magic": 234000,
        "comment": "MACD+Volume M1 Strategy",
        "type_time": mt5.ORDER_TIME_GTC,
        "type_filling": mt5.ORDER_FILLING_IOC,
    }
...
def process_symbol(symbol, last_candle_time, show_logs=False):
    ...
    # New candle detected
    if show_logs:
        print(f"\n[{symbol}] 🕐 New M1 candle at {current_candle_time.strftime('%Y-%m-%d %H:%M:%S')}")
...
def main():
    """Main trading loop"""
    print("="*60)
    print("🚀 MACD + VOLUME LIVE TRADING STRATEGY (M1)")
    print("="*60)
    ...
    try:
        ...
        while True:
            ...
            # Show detailed logs every iteration, summary every 10 iterations (5 minutes)
            show_detailed_logs = True
            show_summary = (iteration % 10 == 1)
            ...
            # Process each symbol
            print(f"\n🔄 Scanning Market (M1) - {current_time}")
            print(f"{'─'*70}")
