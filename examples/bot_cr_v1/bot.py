...
from aiomql.contrib.symbols import CryptoSymbol
from aiomql.contrib.strategies import MACD_Strategy1
...
class BotGUI:
    ...
    def run(self):
        ...
        self.root = tk.Tk()
        self.root.title("🤖 MT5 Crypto Trading Bot - Real-time Logs")
        ...
        # 🔥 KEY FIX: Start bot AFTER GUI is ready
        self.root.after(500, self.start_bot_thread)
        
        self.root.mainloop()
        
    def start_bot_thread(self):
        """Start bot trong background thread"""
        self.add_log("=" * 80)
        self.add_log("🚀 Starting bot in background thread...")
        self.add_log("=" * 80)
        
        # Start bot thread (NOT daemon - will keep running)
        bot_thread = threading.Thread(
            target=run_realtime_bot, 
            args=(self.log_queue, self.control_queue),
            daemon=False  # 🔥 NOT daemon - important!
        )
        bot_thread.start()
    ...
    def setup_ui(self):
        ...
        title_label = tk.Label(
            header_frame, 
            text="📊 MT5 Crypto MACD Trading Bot",
            font=('Arial', 16, 'bold'),
            bg='#2d2d2d',
            fg='#00ff00'
        )
...
# ============================================
# [8] Bot runs in BACKGROUND thread
# ============================================
def run_realtime_bot(log_queue, control_queue):
    """Main bot execution - chạy trong BACKGROUND thread"""
    try:
        log_queue.put("=" * 80)
        ...
        for symbol in loaded_symbols:
            try:
                symbol_obj = CryptoSymbol(name=symbol)
                strategy = MACD_Strategy1(symbol=symbol_obj, config=config)
        ...
        # 🔥 Execute bot - this will block until stopped
        bot.execute()
...
# ============================================
# [9] MAIN ENTRY - GUI runs in MAIN thread
# ============================================
if __name__ == "__main__":
    ...
    # Bot will start automatically after GUI is ready
    gui = BotGUI(log_queue, control_queue)
    gui.run()  # This blocks until window is closed