...
from aiomql.contrib.strategies import EUR_Strategy
...
class BotGUI:
    ...
    def run(self):
        ...
        self.root = tk.Tk()
        self.root.title("🤖 MT5 EUR/USD GBP/USD Bot - Real-time Logs")
    ...
    def setup_ui(self):
        ...
        title_label = tk.Label(
            header_frame, 
            text="📊 MT5 EUR/USD GBP/USD Trading Bot",
            font=('Arial', 16, 'bold'),
            bg='#2d2d2d',
            fg='#00ff00'
        )
...
# ============================================
# [3-6] Các hàm load symbols và check environment
# ============================================
...
async def monitor_bot(config: dict, log_queue, control_queue):
    ...
    while True:
        ...
        await asyncio.sleep(min(interval, 5))  # Check stop signal mỗi 5s
    ...
    try:
        ...
        for symbol in loaded_symbols:
            try:
                ...
                strategy = EUR_Strategy(symbol=symbol_obj, config=config)
        ...
        log_queue.put(f"✅ Total strategies: {len(strategies)}")
        ...
        log_queue.put("🚀 Bot executing with multi-symbol trading...")
        logging.info(f"Bot started with {len(strategies)} strategies")
        
        # Execute bot - này sẽ block cho đến khi stop
        bot.execute()
...
if __name__ == "__main__":
    ...
    # Run bot in MAIN thread (để signals hoạt động)
