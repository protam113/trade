...
print("\n=== First 20 rows EMA & RSI ===")
print(df[["close", "EMA5", "EMA20", "RSI"]].head(20))
...
# === Plot 1: Price + EMAs ===
plot_price = win.addPlot(title="EURUSD M5 Price + EMAs")
...
vol_bars = pg.BarGraphItem(x=[], height=[], width=0.6, brush='g')  # Define vol_bars here
...
# === EMA & RSI Debug Label ===
ema_rsi_label = QtWidgets.QLabel("📈 EMA5: -- | EMA20: -- | RSI: --")
ema_rsi_label.setStyleSheet("color: orange; font-size: 12px;")
layout.addWidget(ema_rsi_label)
...
position = None 
lot = 0.1
ptr = 20  # Start later to allow EMA and RSI initialization
...
def update():
    ...
    for _ in range(speed):
        ...
        if ptr < 20:  # Wait for EMAs and RSI to stabilize
            ptr += 1
            continue
        ...
        # Update EMA & RSI display
        ema_rsi_label.setText(f"📈 EMA5: {ema5_now:.5f} | EMA20: {ema20_now:.5f} | RSI: {rsi_now:.2f}")
        ...
        # === MỞ LỆNH MỚI ===
        if position is None:
            # Check BUY condition - EMA5 crosses above EMA20 and RSI not overbought
            buy_signal = ema5_prev <= ema20_prev and ema5_now > ema20_now and 30 <= rsi_now <= 70
            
            # Check SELL condition - EMA5 crosses below EMA20 and RSI not oversold
            sell_signal = ema5_prev >= ema20_prev and ema5_now < ema20_now and 30 <= rsi_now <= 70
            
            # DEBUG: Log signals
            if debug_mode and (buy_signal or sell_signal):
                if buy_signal:
                    log_event(f"🟡 BUY Signal @ {ptr} | EMA5: {ema5_now:.5f} | EMA20: {ema20_now:.5f} | RSI: {rsi_now:.2f}")
                if sell_signal:
                    log_event(f"🟡 SELL Signal @ {ptr} | EMA5: {ema5_now:.5f} | EMA20: {ema20_now:.5f} | RSI: {rsi_now:.2f}")
